...
- **`Views/MainWindow_Exam.cpp`**: 考试相关 UI 逻辑。
  - 处理模拟考试和自由训练的模式选择。
  - 负责题目显示 (`showQuestion`)、上一题/下一题切换、交卷评分逻辑。
  - 处理倒计时更新。
- **`Views/MainWindow_Review.cpp`**: 错题回顾 UI 逻辑。
  - 负责错题的展示 (`showWrongQuestion`)。
  - 实现视觉反馈：将用户错选标红，正确答案标绿。
  - 处理错题回顾的导航（上一题、下一题、退出）。

## 功能概览

1.  **用户登录**: 输入用户 ID 进入系统。
2.  **管理员登录**: 特定账号密码进入后台管理。
3.  **模拟考试**: 随机抽取题目，限时考试，支持题目向前/向后切换，考完自动评分。
4.  **自由训练**: 顺序练习所有题目，无时间限制，支持题目向前/向后切换。
5.  **错题回顾**: 考试结束后，可查看本次考试的错题，支持前后翻页查看，系统会高亮显示用户错选项（红色）和正确选项（绿色）。
6.  **成绩查询**: 查看历史考试成绩。
